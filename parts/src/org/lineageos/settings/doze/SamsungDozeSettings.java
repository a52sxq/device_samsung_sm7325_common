...
import org.lineageos.settings.utils.ScreenTypeUtils;
...
public class SamsungDozeSettings extends PreferenceFragment
    ...
    public void onCreatePreferences(Bundle savedInstanceState, String rootKey) {
        ...
        mAlwaysOnDisplayPreference = findPreference(DozeUtils.ALWAYS_ON_DISPLAY);
        mAlwaysOnDisplayPreference.setEnabled(dozeEnabled);
        mAlwaysOnDisplayPreference.setChecked(DozeUtils.isAlwaysOnEnabled(getActivity()));
        mAlwaysOnDisplayPreference.setOnPreferenceChangeListener(this);

        mHandwavePreference = findPreference(DozeUtils.GESTURE_HAND_WAVE_KEY);
        mHandwavePreference.setEnabled(dozeEnabled);
        mHandwavePreference.setOnPreferenceChangeListener(this);

        mPickUpPreference = findPreference(DozeUtils.GESTURE_PICK_UP_KEY);
        mPickUpPreference.setEnabled(dozeEnabled);
        mPickUpPreference.setOnPreferenceChangeListener(this);

        mPocketPreference = findPreference(DozeUtils.GESTURE_POCKET_KEY);
        ...
        if (!DozeUtils.alwaysOnDisplayAvailable(getActivity())) {
            ...
            mHandwavePreference.setDependency(DozeUtils.ALWAYS_ON_DISPLAY);
            mPickUpPreference.setDependency(DozeUtils.ALWAYS_ON_DISPLAY);
            mPocketPreference.setDependency(DozeUtils.ALWAYS_ON_DISPLAY);
    ...
    public void onResume() {
        ...
        if (!ScreenTypeUtils.isTablet(getContext())) {
